...
router.post('/', function (req, res, next) {
  ...
  if (req.session.mysession) {
    ...
    if (req.cookies._krol == role_is('campus_role')) {
      ...
      var auth = 'SELECT * FROM depot WHERE Dest = ? AND active="1" ORDER BY ' + order + ', ID ;';
      var entityList = [];
      connection.connect();
      ...
      var auth = 'SELECT * FROM depot WHERE active="1" ORDER BY ' + order + ', ID ;';
      var entityList = [];
      connection.connect();
      connection.query(auth, function (err, rows, fields) {
        if (err) {
          res.status(500).json({ "status_code": 500, "status_message": "internal server error" });
        }
        else {
      ...
      var auth = 'SELECT * FROM depot ORDER BY ' + order + ', ID ;';
      var entityList = [];
      connection.connect();
      connection.query(auth, function (err, rows, fields) {
        if (err) {
