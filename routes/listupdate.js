...
router.post('/', function (req, res, next) {
  ...
  if (req.session.mysession) {
    ...
    if ((req.cookies._krol == role_is('user_role')) || (req.cookies._krol == role_is('admin_role')) || (req.cookies._krol == role_is('campus_role'))) {
      ...
      connection.connect();
      var auth = 'SELECT * FROM list WHERE ID = ?';
      connection.query(auth, updateid, function (err, rows, fields) {
        if (err) {
          ...
          var entity =
          {
            ...
            'Date': rows[0].Date,
            'responsible': rows[0].responsible,
          ...
          var testObj = '{"ID":"' + entity.ID + '", "pType":"' + entity.pType + '", "pName":"' + entity.pName + '", "pBrand":"' + entity.pBrand + '", "responsible":"' + entity.responsible + '", "Date":"' + entity.Date + '", "Detail":"' + entity.Detail + '", "garanti_start":"' + entity.garanti_start + '", "garanti_end":"' + entity.garanti_end + '", "Unit":"' + entity.Unit + '", "Campus":"' + entity.Campus + '", "active":"' + entity.active + '"}';
          var ms = 60 * 60 * 1000;
          res.cookie('CID', entity.ID, { expires: new Date(Date.now() + ms) });
          res.render('listupdate', { "testObj": testObj });
        }
      });
      connection.end();
    }
