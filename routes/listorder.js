...
router.post('/', function (req, res, next) {
  ...
  if (req.session.mysession) {
    ...
    if (req.cookies._krol == role_is('campus_role')) {
      ...
      var entityList = [];
      var auth = 'SELECT * FROM list WHERE   active = "1" AND Campus = ? ORDER BY ' + order + ', ID ;';
      connection.connect();
      ...
      var entityList = [];
      var auth = 'SELECT * FROM list WHERE active = "1" ORDER BY ' + order + ', ID ;';
      connection.connect();
      ...
      var entityList = [];
      var auth = 'SELECT * FROM list ORDER BY ' + order + ', ID ;';
      connection.connect();
      connection.query(auth, function (err, rows, fields) {
        if (err) {
          res.status(500).json({ "status_code": 500, "status_message": "internal server error" });
