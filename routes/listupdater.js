  ...
  if (req.session.mysession) {
    ...
    if ((req.cookies._krol == role_is('user_role')) || (req.cookies._krol == role_is('admin_role')) || (req.cookies._krol == role_is('campus_role'))) {
      ...
      connection.connect(function (err) {
        ...
        var auth = "UPDATE list SET pType = ?, pName = ?, pBrand = ?, responsible = ?, Date = ?, Detail = ?, garanti_start = ?, garanti_end = ?, Unit = ?, Campus = ?, active = ? WHERE ID = ?";

        connection.query(auth, [p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12, changeid], function (err, result) {
          if (err) { console.log(err); }
          else {
